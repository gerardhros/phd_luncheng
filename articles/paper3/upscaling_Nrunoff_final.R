...
r.nue <- terra::rast('E:.../r.nue.tif')
r.nman <- terra::rast('E:.../nofert.tif')
r.nfert <- terra::rast('E:.../nifert.tif')
r.cwheat <- terra::rast('E:.../r.cwheat.tif')
r.cmaize <- terra::rast('E:.../r.cmaize.tif')
r.crice <- terra::rast('E:.../r.crice.tif')
r.cother <- terra::rast('E:.../r.cother.tif')
r.notill <- terra::rast('E:.../r.notill.tif')
r.cropint <- terra::rast('E:.../r.cropint.tif')
r.ccrop <- terra::rast('E:.../r.ccrop.tif')
...
r.dt$nue_ag <- ifelse(r.dt$nue_ag < 0.5, 1, 0)
    ...
    # read data  
    d1 <- readxl::read_xlsx('.../You_paper3_S2.xlsx',sheet = "Nrun")
    d1 <- as.data.table(d1)
    ...
    dt.new[, `eeyes:crop_typerice` := eeyes*crop_typerice]
    dt.new[, `eeyes:crop_typewheat` := eeyes*crop_typewheat]
    ...
    #EE
     ...
     # write as output
     terra::writeRaster(r.fin,'.../scenario_runoff_ee_1.tif', overwrite = TRUE)
    ...
    #CF
     ...
     # write as output
     terra::writeRaster(r.fin,'.../scenario_runoff_cf_2.tif', overwrite = TRUE)
    ...
    #OF
     ...
     # write as output
     terra::writeRaster(r.fin,'.../scenario_runoff_of_3.tif', overwrite = TRUE)
    ...
    #RFR
     ...
     # write as output
     terra::writeRaster(r.fin,'.../scenario_runoff_rfr_4.tif', overwrite = TRUE)
    ...
    #RFT
     ...
     # write as output
     terra::writeRaster(r.fin,'.../scenario_runoff_rft_5.tif', overwrite = TRUE)
     ...
     # write as output
     terra::writeRaster(r.fin,'.../scenario_runoff_nt_6.tif', overwrite = TRUE)
    ...
    # write as output
    terra::writeRaster(r.fin,'.../scenario_runoff_combination.tif', overwrite = TRUE)