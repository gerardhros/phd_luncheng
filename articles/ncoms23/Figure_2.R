...
##### plot response ratio amoong different microbial group within each global change

metaresult_group<-readxl::read_xlsx('Database.xlsx',sheet = "Figure2")
metaresult_group <- as.data.table(metaresult_group)

mydata<- subset(metaresult_group[which(metaresult_group$Group.type=="Data"),], Vari=="NUE")
 ...
 p1<- ggplot(data = mydata,aes(x=Management,y=mean,shape=Group,fill=Group)) +
  ...
  scale_x_discrete(limits=rev(c("Reduced tillage","No tillage","Crop rotation","Cover cropping","Residue retention","Biochar","Fertilizer timing","Fertilizer rate","Fertilizer placement","Organic fertilizer","Combined fertilizer","Enhanced efficiency")),labels = rev(c("RT","ZT","ROT","CC","RES","BC","RFT","RFR","RFP","OF","CF","EE")))+
...
mydata<- subset(metaresult_group[which(metaresult_group$Group.type=="Method"),], Vari=="NUE")
...
ggsave(file = "meta_result.png",width = 410,height = 270, units = "mm")
