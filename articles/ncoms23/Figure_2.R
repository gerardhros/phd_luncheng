...
# plot response ratio among different microbial group within each global change

# read in the excel sheet for the data
metaresult_group<- readxl::read_xlsx('articles/ncoms23/Database.xlsx',sheet = "Figure2")
...
p1<-  ggplot(data = mydata,aes(x=Management,y=mean,shape=Group,fill=Group)) +
      ...
      geom_text(aes(x = Management, y = ci.ub +6, label = n),
                position = position_dodge(width = 0.7),vjust = 0, hjust=0.5, size = 4.5, check_overlap = FALSE)+
...
p2 <- ggplot(data = mydata,aes(x=Management,y=mean,shape=Group,fill=Group)) +
      ...
      geom_errorbar(position=position_dodge(0.7),
                    aes(ymin = ci.lb, ymax = ci.ub), width=0.3,size=0.8)+
      geom_point(position=position_dodge(0.7), size=4, stroke = 0.5) +
...
# save the plot
ggsave(file = "products/meta_result.png",width = 410,height = 270, units = "mm")
